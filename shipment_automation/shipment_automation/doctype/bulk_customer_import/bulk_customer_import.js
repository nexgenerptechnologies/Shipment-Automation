frappe.ui.form.on('Bulk Customer Import', {
    refresh: function (frm) {
        // ── Download Template button ─────────────────────────────
        frm.add_custom_button(__('Download Template'), function() {
            window.open(frappe.urllib.get_full_url(
                "/api/method/shipment_automation.shipment_automation.doctype.bulk_customer_import.bulk_customer_import.download_template"
            ));
        });
        ...
        if (frm.doc.excel_file) {
            frm.add_custom_button(__('Validate Data'), function () {
                frappe.confirm(
                    'Start validation of the uploaded Customer Excel file?',
                    function () {
        ...
        if (frm.doc.status === 'Validated') {
            frm.add_custom_button(__('Process Masters'), function () {
                frappe.confirm(
                    'This will create <b>Customers, Addresses, and Contacts</b> based on the Excel file.',
                    function () {
